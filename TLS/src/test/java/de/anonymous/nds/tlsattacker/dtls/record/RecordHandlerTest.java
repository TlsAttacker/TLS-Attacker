...
public class RecordHandlerTest {
    ...
    anonymous.tlsattacker.tls.record.RecordHandler rh;
	...
	LinkedList<anonymous.tlsattacker.tls.record.Record> testRecords = new LinkedList<>();
	...
	byte[] testOnWireBytes = ArrayConverter
		.hexStringToByteArray("16fefd00000000000000000005012345678914fefd000000000000000100010116fefd00010000000000"
			+ "0200059876543210");
	List<anonymous.tlsattacker.tls.record.Record> result = rh.parseRecords(testOnWireBytes);

	assertEquals("Check number of records", 3, result.size());
	DtlsRecord currentRecord = (anonymous.tlsattacker.dtls.record.DtlsRecord) result.get(0);
	...
	currentRecord = (anonymous.tlsattacker.dtls.record.DtlsRecord) result.get(1);
	...
	currentRecord = (anonymous.tlsattacker.dtls.record.DtlsRecord) result.get(2);
	assertEquals("Check first record type", new Byte(ProtocolMessageType.HANDSHAKE.getValue()), currentRecord
		.getContentType().getValue());
	assertArrayEquals("Check first record protocol version", ProtocolVersion.DTLS12.getValue(), currentRecord
		.getProtocolVersion().getValue());
