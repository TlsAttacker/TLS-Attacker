...
public class DtlsRecordHandler extends RecordHandler {
    ...
    private static final Logger LOGGER = LogManager
	    .getLogger(anonymous.tlsattacker.dtls.record.DtlsRecordHandler.class);
	...
	while (dataPointer != data.length) {
		...
		records.add(new anonymous.tlsattacker.dtls.record.DtlsRecord());
	    }
	    Record record = records.get(currentRecord);
	    // fill record with data
	    dataPointer = fillRecord((anonymous.tlsattacker.dtls.record.DtlsRecord) record, contentType, data,
		    dataPointer);
	...
	for (Record record : records) {
	    anonymous.tlsattacker.dtls.record.DtlsRecord dtlsRecord = (anonymous.tlsattacker.dtls.record.DtlsRecord) record;
     ...
     */
    private int fillRecord(anonymous.tlsattacker.dtls.record.DtlsRecord record, ProtocolMessageType contentType,
	...
	while (dataPointer < rawRecordData.length) {
	    ...
	    anonymous.tlsattacker.dtls.record.DtlsRecord record = new anonymous.tlsattacker.dtls.record.DtlsRecord();
	    record.setContentType(contentType.getValue());
